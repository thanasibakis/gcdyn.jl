...
function main()
	...
	# and also young sequences, which are mostly the naive because germinal
	# centers are still spawning
	filter!(:time => <=(20), tenx_data)
	filter!(:time => >(5), tenx_data)

	# Create a 6-element type space
	discretization_table = compute_quantile_discretization_table(tenx_data.delta_bind_CGG; type_space_size=6)
	...
	discretization_table = compute_quantile_discretization_table(tenx_data.delta_bind_CGG; type_space_size=8)
	type_space = values(discretization_table) |> collect |> sort
	tc_rate_matrix = compute_rate_matrix(tenx_data.sequence, discretization_table, type_space)

	println()
	println("8 types (quantile)")
	println("--------------")
	println("type_space = ", repr(type_space))
	println("discretization_table = ", repr(discretization_table))
	println("Γ = ", repr(tc_rate_matrix))

	# Create a 6-element type space, manually
	discretization_table = compute_manual_discretization_table(tenx_data.delta_bind_CGG, [-Inf, -2, -1, 0, 1, 2, Inf])
	type_space = values(discretization_table) |> collect |> sort
	tc_rate_matrix = compute_rate_matrix(tenx_data.sequence, discretization_table, type_space)

	println()
	println("6 types (manual)")
	println("--------------")
	println("type_space = ", repr(type_space))
	println("discretization_table = ", repr(discretization_table))
	println("Γ = ", repr(tc_rate_matrix))

	# Create a 8-element type space, manually
	discretization_table = compute_manual_discretization_table(tenx_data.delta_bind_CGG, [-Inf, -2, -1, -0.5, 0, 0.5, 1, 2, Inf])
	type_space = values(discretization_table) |> collect |> sort
	tc_rate_matrix = compute_rate_matrix(tenx_data.sequence, discretization_table, type_space)

	println()
	println("8 types (manual)")
	println("--------------")
	println("type_space = ", repr(type_space))
	println("discretization_table = ", repr(discretization_table))
	println("Γ = ", repr(tc_rate_matrix))
